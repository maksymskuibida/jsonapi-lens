...
interface Loaded {
  ...
  parseMs: number;
  /**
   * The request/response context attached to this document, when any has been
   * entered. Never `undefined` itself — `hasExchangeContent`
   * (render-request.ts) is how "nothing entered" is tested, not this field's
   * presence.
   */
  exchange: Exchange;
}
...
async function load(text: string, label: string, options: LoadOptions): Promise<boolean> {
  ...
  try {
    current = readLoaded(text, label, options.exchange ?? {}, () =>
