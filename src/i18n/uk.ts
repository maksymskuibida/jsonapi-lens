...
export const uk: Messages = {
  ...
  dangling: {
    ...
    noteJson:
      "Кожен із цих ключів читається як посилання — назва, що закінчується на `id`, — але під відповідною назвою нічого не знайшлося. Можливо, документ не має другої половини; можливо, дві назви не відповідають одна одній так, як їх зіставляє це прочитання; можливо, ключ узагалі не є посиланням.",
  },
  ...
  share: {
    ...
    note: "Будь-хто з посиланням може прочитати документ, тож поводьтеся з ним, як із самим payload. Ключ — це частина після #: браузер тримає її в себе й ніколи не надсилає на сервер. Але вона все одно потрапляє в історію браузера та всюди, куди вставлено посилання, — надсилайте його так само, як надсилали б payload.",
    redacting: (n) =>
      f.plural(n, {
        one: "1 значення в доданому запиті схоже на облікові дані — його буде вилучено до створення посилання.",
        few: `${f.n(n)} значення в доданому запиті схожі на облікові дані — їх буде вилучено до створення посилання.`,
        many: `${f.n(n)} значень у доданому запиті схожі на облікові дані — їх буде вилучено до створення посилання.`,
        other: `${f.n(n)} значення в доданому запиті схожі на облікові дані — їх буде вилучено до створення посилання.`,
      }),
  ...
  shareErrors: {
    ...
    damaged: {
      headline: "У цьому спільному посиланні бракує ключа.",
      hint: "Спільне посилання закінчується на # і ключ — а це ні. Деякі месенджери, скорочувачі посилань і поштові фільтри його обрізають. Попросіть посилання ще раз і скопіюйте його повністю.",
    },
    corruptShort: {
  ...
  bundle: {
    errors: {
      ...
      reloaded:
        "Ці спільні документи більше не завантажені. Ключ посилання видаляється з адресного рядка одразу після відкриття, тож перезавантаження не може отримати їх знову — відкрийте початкове посилання, щоб імпортувати їх.",
    },
  },
  ...
  request: {
      ...
      params: {
        convention: (c) => {
          switch (c) {
            ...
            case "truncated":
              return "надто глибока вкладеність, щоб розкодувати";
          }
        },
    ...
    form: {
      ...
      detach: "Видалити запит",
      detachTitle: "Повністю від'єднати запит і відповідь від цього документа",
      detachConfirm: "Запит і відповідь буде від'єднано від цього документа. Це незворотно.",
