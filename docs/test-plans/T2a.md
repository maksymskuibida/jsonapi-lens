...
| 26a | A `Set-Cookie` line whose **attribute value** carries a credential, because an unsplit second cookie landed inside `Path` | The attribute is redacted and counted, the secret appears nowhere in the serialised exchange, and an ordinary `Domain` beside it is untouched. Guards the fix: stubbing `attributeLooksUnsafe` to `false` turns this red | automated (vitest) |
...
| 34a | A credential carried **in the fragment itself** (`#access_token=<jwt>&state=xyz`) — the OAuth implicit-flow case | The secret is absent from the returned URL **string**, counted once, and the non-credential remainder of the fragment survives. Guards the fix rather than merely accompanying it: reverting fragment redaction turns this red | automated (vitest) |
| 34b | A fragment carrying no credential (`#section-2`) | URL returned byte-identical — the fragment is not collateral damage | automated (vitest) |
