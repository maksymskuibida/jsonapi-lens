...
| T7 · MCP server | 🔨 | [T7](task-specs/T7.md) | Branch `feat/T7-mcp-server`, based on `feat/T5-share-envelope` (T7 depends on T5's envelope; PR base matches). A stdio `mcp/` server, `share`/`read`, importing `src/crypto.ts` unmodified — no second implementation. `share` takes a document list and a caller-supplied `openssl rand -hex 32` secret, seals one document or a bundle, uploads it, and returns the id and the built URL; `read` fetches, decrypts and returns the document(s) verbatim. Adds `mcp/tsconfig.json` to the verify chain and to `deploy.yml`'s `check` job. |
