...
| T6 · Share a bundle | 🔨 | [T6](task-specs/T6.md) | [PR #5](https://github.com/maksymskuibida/jsonapi-lens/pull/5), branch `feat/T6-share-bundle`, based on `feat/T5-share-envelope` (opens against it, not `main`, until T5 merges). Selection mode in `openLibraryModal`: `Share` puts a checkbox on each row and swaps the footer for `Cancel`/`Create link`, disabled with nothing ticked; one tick mints today's version-2 link via the existing share modal, several mint a version-3 bundle through the same modal. A new bundle import view (`src/bundle.ts`) lists a decrypted bundle's documents, ticked by default except anything byte-identical to an existing saved document, and writes the ticked ones to the library. `mintShareEnvelope` in `bundle.ts` is [D3](DECISIONS.md) — T7 should reuse it rather than re-deriving the one-vs-many decision. |
| T7 · MCP server | 🔨 | [T7](task-specs/T7.md) | [PR #3](https://github.com/maksymskuibida/jsonapi-lens/pull/3), branch `feat/T7-mcp-server`, based on `feat/T5-share-envelope` (T7 depends on T5's envelope; PR base matches). A stdio `mcp/` server, `share`/`read`, importing `src/crypto.ts` unmodified — no second implementation. `share` takes a document list and a caller-supplied `openssl rand -hex 32` secret, seals one document or a bundle, uploads it, and returns the id and the built URL; `read` fetches, decrypts and returns the document(s) verbatim. Adds `mcp/tsconfig.json` to the verify chain and to `deploy.yml`'s `check` job. |
...
| 2 | **T2** ‖ **T6** ‖ **T7** ‖ **T8** | T2: the new request modules, `main.ts`, `render-document.ts`, `types.ts`, `styles.css`. T6: `panels.ts`'s selection mode, the new `src/bundle.ts`, `share.ts` extended (an optional `exchange` param, the chrome factored into a shared `runShareModal` — see [D3](DECISIONS.md)), and a two-place `main.ts` hook (a `showView` case, the `loadSharedDocument`/`applyRoute` branch). T7: a new `mcp/` tree only. T8: `format.ts`, `i18n/intl.ts`, `router.ts`, `seo.ts`, `render-resource.ts`. T2 and T6 both touch `main.ts` in different regions; second to land rebases. **T8 runs after T1 merges** — it edits files T1 owns. D4 goes to whichever of T6/T8 gets there first; see T8's ownership note. |
