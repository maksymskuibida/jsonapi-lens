...
| 2 | **T2** ‖ **T6** ‖ **T7** | T2: the new request modules + the document view. T6: `panels.ts`'s selection mode, the new `src/bundle.ts`, `share.ts` extended (an optional `exchange` param, the chrome factored into a shared `runShareModal` — see [D3](DECISIONS.md)), and a two-place `main.ts` hook (a `showView` case, the `loadSharedDocument`/`applyRoute` branch). T7: a new `mcp/` tree only. T2 and T6 both touch `main.ts` in different regions; second to land rebases. |
