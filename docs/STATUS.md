...
| T10 · Locale-independent test suite | 🔨 | [T10](task-specs/T10.md) | Pre-existing defect: Node's built-in `navigator` reflects the host locale, so `src/crypto.ts`'s `t()`-read refusals answered in German on a German machine and `test/crypto.test.ts` failed 5 of its assertions — on the two languages this app itself ships. Pinned once for the whole suite in `test/setup.ts` through the `stored()` step `locale()` consults before `navigator`, rather than per assertion. Also fixes `test/mcp/locale-pin.test.ts`'s positive control, which set `LANG` on its child but inherited the runner's `LC_ALL` — the reason it failed under `C`, which is what CI uses. 618/618 under de/uk/en/C. [PR #7](https://github.com/maksymskuibida/jsonapi-lens/pull/7). |
| T1 · Plain JSON | ✅ | [T1](task-specs/T1.md) | [PR #2](https://github.com/maksymskuibida/jsonapi-lens/pull/2) merged. Shape detection, the branch out of `assertJsonApi`, an **inferred identity graph** — a repeated identifier becomes a link, the way a `{type, id}` pointer already does — and the [D1](DECISIONS.md) anchor scope table. |
| T5 · Storage + share envelope | ✅ | [T5](task-specs/T5.md) | [PR #1](https://github.com/maksymskuibida/jsonapi-lens/pull/1) merged. An optional exchange on stored documents and share payloads, IndexedDB v3, and a **bundle** payload at envelope version 3. No UI, so T2/T6/T7 can build on it in parallel. |
...
| T6 · Share a bundle | 🔨 | [T6](task-specs/T6.md) | [PR #5](https://github.com/maksymskuibida/jsonapi-lens/pull/5), branch `feat/T6-share-bundle`, based on `feat/T5-share-envelope` (opens against it, not `main`, until T5 merges). Selection mode in `openLibraryModal`: `Share` puts a checkbox on each row and swaps the footer for `Cancel`/`Create link`, disabled with nothing ticked; one tick mints today's version-2 link via the existing share modal, several mint a version-3 bundle through the same modal. A new bundle import view (`src/bundle.ts`) lists a decrypted bundle's documents, ticked by default except anything byte-identical to an existing saved document, and writes the ticked ones to the library. `mintShareEnvelope` in `bundle.ts` is [D5](DECISIONS.md) — T7 should reuse it rather than re-deriving the one-vs-many decision. |
...
| 2 | **T2** ‖ **T6** ‖ **T7** ‖ **T8** | T2: the new request modules, `main.ts`, `render-document.ts`, `types.ts`, `styles.css`. T6: `panels.ts` + the bundle route. T7: a new `mcp/` tree only. T8: `format.ts`, `i18n/intl.ts`, `router.ts`, `seo.ts`, `render-resource.ts`. T2 and T6 both touch `main.ts` in different regions; second to land rebases. **T8 runs after T1 merges** — it edits files T1 owns. D4 went to T2a (the parameter model) and T6 took D5; see DECISIONS.md. |
