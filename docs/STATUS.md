...
| T2 · Exchange, form and review | 🔨 | [T2](task-specs/T2.md) | Split into two pull requests — see the split note at the top of the spec. **T2a** (branch `feat/T2a-exchange-model`): the `Exchange`/`RequestPart`/`ResponsePart`/`BodyPart` model filling in T5's [D2](DECISIONS.md) placeholder, `mergeExchange`, the parameter decoder (`params.ts`, every row of the spec's table plus JSON:API's `include`/`fields`/`sort`/`page`/`filter`), header/cookie parsing, and secret detection/JWT decoding/redaction (`secrets.ts`). **T2b** (this PR, branch `feat/T2b-form-and-review`, built on T2a): the field-separated form (`src/request-form.ts`, a modal), the band and three-mode review (`src/render-request.ts`), the `q_`/`b_`/`d_` anchor scopes wired in for the first time (the table itself was already exhaustive from T1), and persistence/share wiring in `main.ts`. `json-index.ts#buildJsonIndex` and `render-json.ts#buildAnnotations` each gained a small, additive, optional scope parameter (default unchanged) so a plain-JSON request body can reuse the collection/identity machinery under `d_` instead of duplicating it. **Known gap, flagged rather than silently shipped:** the existing single-document Share flow (`share.ts#openShareModal`) does not yet pass `current.exchange` into the sealed payload — that needs a small additive parameter there, and `share.ts` is outside this PR's assigned files. Copy and Download are fully wired with redaction and an honest, non-overclaiming count; the redact→seal→open round trip is proven directly in `test/exchange-redaction.test.ts` against `crypto.ts`'s real primitives. See the PR body for the exact follow-up. |
